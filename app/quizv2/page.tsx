...
// Loading animation component - minimalistic and clean
function CalculatingQuotes({ firstName }: { firstName: string }) {
  return (
    <div className="animate-in fade-in duration-500 flex flex-col items-center justify-center py-12">
      {/* Spinning circle */}
      <div className="w-12 h-12 border-4 border-slate-200 border-t-primary rounded-full animate-spin mb-8" />
      
      <h2 className="text-2xl sm:text-3xl font-bold text-foreground mb-3 text-center">
        Calculating Best Quotes
      </h2>
...
function QuizContent() {
  ...
  const handleOptionSelect = (field: string, value: string) => {
    ...
    setTimeout(() => {
      // If we're on step 5 (income/reason is step 5), show loading animation before step 6
      if (currentStep === 4) {
        // This is step 4 (DOB/Gender), moving to step 5
        setCurrentStep(currentStep + 1)
      } else if (currentStep === 5) {
        // Wait for both income and reason to be filled before showing loading
        // This is handled in handleNext for step 5
      } else {
        setCurrentStep(currentStep + 1)
      }
    }, 250)
  ...
  const handleNext = async () => {
    // After step 5 (income/reason), show loading animation
    if (currentStep === 5) {
      setShowLoadingAnimation(true)
      // Show animation for 4 seconds then proceed to contact form
      setTimeout(() => {
    ...
    if (currentStep < totalSteps - 1) {
      ...
      try {
        const payload = {
          ...
          variant: "v2", // Track which variant this came from
        }
        ...
        // Fire Google Ads conversion (only if income is NOT "Under $30,000")
      ...
      router.push(`/quizv2/thank-you?zip=${zipCode}&name=${formData.firstName}`)
    }
  }
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="w-full bg-slate-200 h-2 shrink-0">
        <div 
          ...
          style={{ width: `${showLoadingAnimation ? 85 : progress}%` }}
        />
      </div>
      ...
      <main className="flex-1 flex flex-col items-center px-5 py-8 md:py-10">
        ...
        >
          ...
          {/* Loading Animation (shown between step 5 and 6) */}
          {showLoadingAnimation && (
            <CalculatingQuotes firstName={formData.firstName} />
          )}

          {/* Step 0: Currently Insured */}
          {!showLoadingAnimation && currentStep === 0 && (
          ...
          {!showLoadingAnimation && currentStep === 1 && (
          ...
          {!showLoadingAnimation && currentStep === 2 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-8 leading-tight">
          ...
          {/* Step 3: Name */}
          {!showLoadingAnimation && currentStep === 3 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          {/* Step 4: DOB & Gender */}
          {!showLoadingAnimation && currentStep === 4 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          {!showLoadingAnimation && currentStep === 5 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          {/* Step 6: Contact Info */}
          {!showLoadingAnimation && currentStep === 6 && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-2 leading-tight">
